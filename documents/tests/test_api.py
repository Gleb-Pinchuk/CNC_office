...
class TestDocumentsAPI:
    ...
    def test_share_document_updates_existing_permission(self, auth_client, document):
        ...
        other = get_user_model().objects.create_user(username='doc_writer', password='pass')
        FilePermission.objects.create(file_type='document', file_id=document.id, user=other, permission='read')
        ...
        assert res.status_code == status.HTTP_200_OK
        perm = FilePermission.objects.get(file_type='document', file_id=document.id, user=other)
    ...
    def test_save_content_allows_write_share(self, document):
        ...
        other = get_user_model().objects.create_user(username='doc_editor', password='pass')
        FilePermission.objects.create(file_type='document', file_id=document.id, user=other, permission='write')
