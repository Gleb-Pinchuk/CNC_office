...
class TestSectionTablesAPI:
    ...
    def test_create_table_uses_section_type_from_request(self, auth_client):
        res = auth_client.post(
            '/api/section-tables/',
            data={'title': 'New Rangers Table', 'section_type': 'rangers', 'content': {}},
            format='json',
        )
    ...
    def test_share_table_updates_existing_permission(self, auth_client, section_table):
        ...
        other = get_user_model().objects.create_user(username='table_reader', password='pass')
        FilePermission.objects.create(file_type='section_table', file_id=section_table.id, user=other, permission='read')
        ...
        assert res.status_code == status.HTTP_200_OK
        perm = FilePermission.objects.get(file_type='section_table', file_id=section_table.id, user=other)
    ...
    def test_save_content_allows_write_share(self, section_table):
        ...
        other = get_user_model().objects.create_user(username='table_editor', password='pass')
        FilePermission.objects.create(file_type='section_table', file_id=section_table.id, user=other, permission='write')
    ...
    def test_by_section_returns_only_current_users_tables(self, auth_client, user, section_table):
        from django.contrib.auth import get_user_model
        from sections.models import SectionTable

        other = get_user_model().objects.create_user(username='other_owner', password='pass')
        SectionTable.objects.create(owner=other, title='Other table', section_type='attendance', content={})

        res = auth_client.get(f'/api/section-tables/by_section/{section_table.section_type}/')
