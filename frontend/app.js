...
function updateBreadcrumb() {
    ...
    if (currentFolder) {
        html += ` <span style="color: #6B7280; margin: 0 0.25rem;">/</span> <span style="font-weight: 600;">${escapeHtml(currentFolder.name)}</span>`;
    }
...
function renderFiles(files) {
    ...
    files.forEach((file, index) => {
        if (file) filesGrid.appendChild(createFileCard(file, index));
    });
}
...
function createFileCard(file, index) {
    ...
    if (file.file_name) fileName = file.file_name;
        ...
        try {
            fileName = decodeURIComponent(file.file.split('/').pop());
        } catch (e) {}
    }
...
async function uploadFile(file) {
    ...
    try {
        const headers = getAuthHeaders(false); // Не добавляем Content-Type для FormData
...
function renderFolders(folders) {
    ...
    folders.forEach((folder, index) => {
        filesGrid.appendChild(createFolderCard(folder, index));
    });
}
...
async function loadShared() {
    ...
    try {
        ...
        if (response.status === 200) {
            renderShared(await response.json());
        } else if (response.status === 401 || response.status === 403) {
            hideLoading();
            clearAuth();
            showLoginModal();
...
function renderShared(permissions) {
    ...
    permissions.forEach((perm, index) => {
        filesGrid.appendChild(createSharedCard(perm, index));
    });
}
...
function renderLogs(logs) {
    ...
    logs.forEach((log, index) => {
        filesGrid.appendChild(createLogCard(log, index));
    });
}
...
function createLogCard(log, index) {
    ...
    card.innerHTML = `
        ...
        <div class="file-meta"><span>${log.user_username || 'Система'}</span><span>${new Date(log.timestamp).toLocaleString('ru-RU')}</span></div>
        ${details}
    `;
